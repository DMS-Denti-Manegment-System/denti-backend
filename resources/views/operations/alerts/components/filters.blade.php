<x-page-toolbar>
    ...
    <div class="col-md-3 col-lg-2"><label class="form-label">Tip</label><select name="type" class="form-select form-select-solid"><option value="">Tum tipler</option>@foreach(['low_stock' => 'Dusuk stok', 'critical_stock' => 'Kritik stok', 'near_expiry' => 'Yaklasan SKT', 'critical_expiry' => 'Kritik SKT', 'expired' => 'Suresi gecmis'] as $value => $label)<option value="{{ $value }}" @selected(request('type') === $value)>{{ $label }}</option>@endforeach</select></div>
    ...
    <div class="col text-end align-self-end d-flex gap-2 justify-content-end">
        <a href="{{ route('alerts.sync') }}" class="btn btn-light-info">
